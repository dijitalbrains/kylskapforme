@props(['rangeMin', 'rangeMax', 'min', 'max'])

<div {{ $attributes->merge(['class' => 'flex flex-col items-center']) }}
    x-data="rangeSlider({{$rangeMin}}, {{$rangeMax}}, {{$min}}, {{$max}})">
    <input class="absolute opacity-0 pointer-events-none" type="range" name="price_min" min="{{$min}}" max="{{$max}}"
        x-model="min" class="px-3 py-2 rounded" step="100">
    <input class="absolute opacity-0 pointer-events-none" type="range" name="price_max" min="{{$min}}" max="{{$max}}"
        x-model="max" step="100">

    <div class="flex items-center relative bg-white w-full h-1.5 mt-4" x-ref="sliderEl"
        @touchend.window="dragLeft = dragRight = false" @touchmove.window="handleThumbMouseMove($event)"
